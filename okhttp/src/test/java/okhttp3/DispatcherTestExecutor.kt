...
import okhttp3.internal.concurrent.RecordingExecutor
import org.assertj.core.api.Assertions.assertThat

internal class DispatcherTestExecutor(
  private val dispatcherTest: DispatcherTest
) : RecordingExecutor() {
  fun assertJobs(vararg expectedUrls: String) {
    val actualUrls = calls.map { (it as RealCall.AsyncCall).request().url.toString() }
  ...
  fun finishJob(url: String) {
    ...
    while (i.hasNext()) {
      val call = i.next() as RealCall.AsyncCall
