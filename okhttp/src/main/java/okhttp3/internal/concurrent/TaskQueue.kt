...
class TaskQueue internal constructor(
  ...
  fun schedule(task: Task, delayNanos: Long) {
    ...
    synchronized(taskRunner) {
      if (scheduleAndDecide(task, delayNanos)) {
        taskRunner.kickPromoter(this)
      }
    }
  }

  /** Returns true if the promoter should run. */
  private fun scheduleAndDecide(task: Task, delayNanos: Long): Boolean {
    ...
    // Run the promoter if we inserted at the front.
    return insertAt == 0
  }
  ...
  private fun cancelAllAndDecide(): Boolean {
    ...
    // Run the promoter if tasks were canceled.
    return cancelTasks.isNotEmpty()
  }

  /**
   * Start executing the next available task for [queue].
   *
  ...
  internal fun promote(
    ...
    if (cancelTask != null) {
      ...
      executor.execute(cancelTask.cancelRunnable!!)
    ...
    if (delayNanos <= 0) {
      ...
      executor.execute(runTask.runRunnable!!)
